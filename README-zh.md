...
*「时痕 · STE」（原「时迹 · TimeTrace」，2026-08-27 更名）项目的 schema 仓库——一张承载时间的地图，每个地点都有自己的历史。*
...
[**曾经的母校**](https://wonderingWu.github.io/ste-schema/demo/) —— 由 STE 数据驱动的可用地图 Demo：

- **89 所有完整时间轴的学校**：北京/上海 77 所有据可查的消失、更名、迁址中小学（1950s–2026），外加 12 所历史名校——包括西南联大这类战时迁址实体，迁址用快照级坐标表达
- **全国图**为第一视图，可按城市下钻；时间滑块（1879 至今）让学校在地图上沿时间轴出现、改名、搬迁、消失
- 访客可以通过 `ste-memo` 协议为学校留下记忆（文字与照片），也可以为尚未收录的学校留名——留名数据驱动下一座开哪座城
- 精选数据包在 `demo/data/`（全部通过 v0.2 校验）。石家庄包是**明确标注的合成演示数据**，用于体验贡献流程；京沪与历史名校包均为有据可查的真实学校
